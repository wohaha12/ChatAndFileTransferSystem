...
struct TransHeader {
  ...
  TransHeader(uint32_t cmd = 0, uint32_t seq = 0, uint32_t len = 0)
      : magic(0x12345678), version(1), cmd(cmd), seq(seq), len(len),
        checksum(0), reserved(0) {}
  ...
  static bool validateChecksum(const char *data, uint32_t dataLen,
  ...
  /**
   * @brief 验证校验和（成员函数版，自动使用从头部解析的 len 和 checksum）
   * @param data 包体数据
   * @return true-校验成功，false-校验失败
   */
  bool validateChecksum(const char *data) const {
    return validateChecksum(data, len, checksum);
  }
   ...
   */
  bool validateMagic() const { return magic == 0x12345678; }
};
