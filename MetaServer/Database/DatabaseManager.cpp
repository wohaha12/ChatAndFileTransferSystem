...
DBConnection::DBConnection(MYSQL* mysql)
    : m_mysql(mysql)
{
}
...
bool DatabaseManager::initialize(const std::string& host, uint16_t port, const std::string& user, 
                                ...
                                const std::string& charset, uint32_t poolSize)
{
...
MYSQL* DatabaseManager::getConnection()
{
    ...
    if (m_cond.wait_for(lock, std::chrono::seconds(5), [this] {
        return !m_freeConnections.empty();
    })) {
        MYSQL* conn = m_freeConnections.front();
        m_freeConnections.pop();
...
void DatabaseManager::getPoolStatus(uint32_t& totalSize, uint32_t& freeSize, uint32_t& usedSize) const
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
MYSQL* DatabaseManager::createConnection()
{
    ...
    if (!mysql_real_connect(mysql, m_host.c_str(), m_user.c_str(), 
                         ...
                         m_port, nullptr, CLIENT_MULTI_STATEMENTS)) {
