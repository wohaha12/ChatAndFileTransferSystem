    ...
    , ui(new Ui::ServerWindow)
{
    ...
    connect(timer, &QTimer::timeout, this, [this]() {
        ui->statusLabel->setText(QString::fromStdString("运行中 | 时间: " + 
                                           ...
                                           float cpuUsage, float memoryUsage, float diskUsage)
{
    ...
    QString serverInfo = QString("服务器 %1: %2 | CPU: %3% | 内存: %4% | 磁盘: %5%")
        .arg(serverId)
        .arg(statusText)
        .arg(cpuUsage, 0, 'f', 1)
...
void ServerWindow::addLog(const std::string& log)
{
    ...
    QString logEntry = QString("[%1] %2").arg(timestamp).arg(QString::fromStdString(log));
...
{
    QString aboutInfo = QString(
        "聊天与文件传输系统 - MetaServer\n"
        "版本: 1.0.0\n"
        "基于Qt 5.14.2 + C++17开发\n"
        "\n"
        "功能特性:\n"
...
void ServerWindow::initializeConnections()
{
    ...
    connect(ui->configButton, &QPushButton::clicked,
            this, &ServerWindow::onConfigClicked);
    connect(ui->aboutButton, &QPushButton::clicked,
            this, &ServerWindow::onAboutClicked);
