                                      ...
                                      const std::string& fileHash, uint64_t parentId)
{
    ...
    std::ostringstream oss;
    oss << "INSERT INTO t_file_info (user_id, file_name, file_path, file_size, file_hash, parent_id) "
           "VALUES (" << userId << ", '" << fileName << "', '" << filePath << "', "
           << fileSize << ", '" << fileHash << "', " << parentId << ")";
...
std::vector<FileMetaMgr::FileMetaInfo> FileMetaMgr::getUserFiles(uint64_t userId, uint64_t parentId)
{
    ...
    oss << "SELECT id, user_id, file_name, file_path, file_size, file_hash, "
           ...
           "FROM t_file_info WHERE user_id = " << userId << " AND parent_id = " << parentId
           << " AND is_deleted = 0";
...
bool FileMetaMgr::moveFile(uint64_t userId, uint64_t fileId, uint64_t newParentId)
{
    ...
    if (affectedRows > 0) {
        emit fileRecordChanged(userId, fileId);
        std::cout << "移动文件成功: " << fileId << " -> parent: " << newParentId << std::endl;
    }
