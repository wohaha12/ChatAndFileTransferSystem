...
void TcpServer::handleNewConnection(qintptr socketDescriptor)
{
    ...
    connect(connection, &Connection::messageReceived, this, [this, socketDescriptor](const Protocol::TransHeader& header, const QByteArray& data) {
        emit messageReceived(socketDescriptor, header, data);
    });
    
    connect(connection, &Connection::connectionClosed, this, [this, socketDescriptor]() {
        handleConnectionClose(socketDescriptor);
    });
