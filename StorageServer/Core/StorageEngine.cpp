...
StorageEngine::StorageEngine(QObject *parent) : QObject(parent) {}
 ...
 * @return 安全的文件路径
 */
std::string StorageEngine::sanitizeFilePath(const std::string &filePath) {
...
QByteArray StorageEngine::readChunk(const std::string &filePath,
                                    uint32_t chunkIndex, uint32_t chunkSize) {
  std::lock_guard<std::mutex> lock(m_mutex);
...
bool StorageEngine::createFile(const std::string &filePath, uint64_t fileSize) {
  ...
  std::cout << "创建文件成功: " << fullPath << " (大小: " << fileSize
            << " 字节)" << std::endl;
  return true;
}
...
std::string
StorageEngine::calculateFileHash(const std::string &filePath) const {
  std::string safePath = sanitizeFilePath(filePath);
  std::string fullPath = m_basePath + "/" + safePath;

  std::string hash;
  if (!ChatSystem::Utils::HashUtil::calculateFileSHA256(fullPath, hash)) {
    std::cerr << "计算文件哈希失败: " << fullPath << std::endl;
    return "";
  }
  return hash;
}
...
#endif
    std::cerr << "创建目录失败: " << dirPath << " (" << strerror(errno) << ")"
              << std::endl;
    return false;
  }
