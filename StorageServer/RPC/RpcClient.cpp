...
RPCClient::RPCClient(QObject *parent)
    : QObject(parent), m_socket(nullptr), m_connected(false),
      m_uploadRecordDB(nullptr), m_retryTimer(nullptr) {
  m_socket = new QTcpSocket(this);

  connect(m_socket, &QTcpSocket::readyRead, this,
          &RPCClient::onSocketReadyRead);
  connect(m_socket, &QTcpSocket::errorOccurred, this,
          &RPCClient::onSocketError);
  connect(m_socket, &QTcpSocket::disconnected, this,
          &RPCClient::onSocketDisconnected);
...
bool RPCClient::connectToServer(const std::string &host, uint16_t port) {
  ...
  if (!m_socket->waitForConnected(5000)) {
    std::cerr << "连接服务器失败: " << m_socket->errorString().toStdString()
              << std::endl;
    return false;
  }
...
bool RPCClient::sendHeartbeat(uint32_t serverId, float cpuUsage,
  ...
  QByteArray requestBytes(reinterpret_cast<const char *>(&request),
                          sizeof(request));

  return sendRequest(requestBytes);
}

bool RPCClient::sendUploadComplete(uint64_t userId, uint64_t fileId,
  ...
  strncpy(notification.upload_token, uploadToken.c_str(),
          sizeof(notification.upload_token) - 1);
  notification.upload_token[sizeof(notification.upload_token) - 1] = '\0';
  notification.user_id = userId;
  // 注: UploadCompleteNotify 没有 fileId 字段，使用其他方式标识文件
  strncpy(notification.file_hash, fileHash.c_str(),
          sizeof(notification.file_hash) - 1);
  notification.file_hash[sizeof(notification.file_hash) - 1] = '\0';
  notification.file_size = fileSize;
  strncpy(notification.storage_path, storagePath.c_str(),
          sizeof(notification.storage_path) - 1);
  ...
  QByteArray notificationBytes(reinterpret_cast<const char *>(&notification),
                               sizeof(notification));
...
void RPCClient::onRetryTimer() { retryPendingNotifications(); }

void RPCClient::retryPendingNotifications() {
  std::vector<LocalUploadRecord> records =
      m_uploadRecordDB->getPendingRetryRecords(RETRY_INTERVAL_MS / 1000);

  if (records.empty()) {
    return;
  }

  std::cout << "开始重试 " << records.size() << " 条待确认的上传记录"
            << std::endl;
  ...
  for (const auto &record : records) {
    if (record.retryCount >= MAX_RETRY_COUNT) {
      ...
      m_uploadRecordDB->updateUploadRecordStatus(
          record.uploadToken, 2, record.retryCount, "超过最大重试次数");
      continue;
    }

    Protocol::UploadCompleteNotify notification;
    strncpy(notification.upload_token, record.uploadToken.c_str(),
            sizeof(notification.upload_token) - 1);
    notification.upload_token[sizeof(notification.upload_token) - 1] = '\0';
    notification.user_id = record.userId;
    strncpy(notification.file_hash, record.fileHash.c_str(),
            sizeof(notification.file_hash) - 1);
    notification.file_hash[sizeof(notification.file_hash) - 1] = '\0';
    notification.file_size = record.fileSize;
    strncpy(notification.storage_path, record.storagePath.c_str(),
            sizeof(notification.storage_path) - 1);
    notification.storage_path[sizeof(notification.storage_path) - 1] = '\0';
    ...
    notification.reserved = 0;
    strncpy(notification.error_msg, record.errorMsg.c_str(),
            sizeof(notification.error_msg) - 1);
    ...
    if (!success) {
      std::cerr << "重试上传完成通知失败，更新重试次数: " << record.uploadToken
                << std::endl;
      m_uploadRecordDB->updateUploadRecordStatus(
          record.uploadToken, 0, record.retryCount + 1, "重试失败");
    }
  }
}
...
bool RPCClient::sendRequest(const QByteArray &request) {
  ...
  if (bytesWritten == -1) {
    std::cerr << "发送请求失败: " << m_socket->errorString().toStdString()
              << std::endl;
    return false;
  }
