...
UploadRecordDB::~UploadRecordDB() { close(); }
...
bool UploadRecordDB::initialize(const std::string &dbPath) {
  ...
  if (!m_db.open()) {
    std::cerr << "打开数据库失败: " << m_db.lastError().text().toStdString()
              << std::endl;
    return false;
  }
...
bool UploadRecordDB::createTables() {
  ...
  if (!query.exec(createTableSql)) {
    std::cerr << "创建表失败: " << query.lastError().text().toStdString()
              << std::endl;
    return false;
  }
  ...
  QStringList indexSqls = {
      ...
      "CREATE INDEX IF NOT EXISTS idx_upload_time ON "
      "upload_records(upload_time);",
      "CREATE INDEX IF NOT EXISTS idx_last_retry ON "
      "upload_records(last_retry_time);"};

  for (const QString &indexSql : indexSqls) {
    if (!query.exec(indexSql)) {
      std::cerr << "创建索引失败: " << query.lastError().text().toStdString()
                << std::endl;
      return false;
    }
  }
...
bool UploadRecordDB::saveUploadRecord(const LocalUploadRecord &record) {
  ...
  if (!query.exec()) {
    std::cerr << "执行SQL语句失败: " << query.lastError().text().toStdString()
              << std::endl;
    return false;
  }
...
bool UploadRecordDB::updateUploadRecordStatus(const std::string &token,
                                              uint32_t status,
                                              uint32_t retryCount,
  ...
  std::cout << "更新上传记录状态成功: " << token << " (状态: " << status << ")"
            << std::endl;
  return true;
}

std::vector<LocalUploadRecord>
UploadRecordDB::getPendingRetryRecords(uint64_t retryInterval) {
  std::lock_guard<std::mutex> lock(m_mutex);
  ...
  if (!query.exec()) {
    std::cerr << "准备SQL语句失败: " << query.lastError().text().toStdString()
              << std::endl;
    return records;
  }
...
bool UploadRecordDB::deleteConfirmedRecords(int days) {
  ...
  uint64_t expireTime =
      static_cast<uint64_t>(std::time(nullptr)) - (days * 86400);
