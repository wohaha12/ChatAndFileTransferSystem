...
signals:
  void uploadProgress(const QString &filePath, qint64 bytesSent,
                      qint64 bytesTotal);
  void uploadFinished(const QString &filePath, bool success,
                      const QString &errorMsg);
  void downloadProgress(uint64_t fileId, qint64 bytesReceived,
                        qint64 bytesTotal);
  void downloadFinished(uint64_t fileId, bool success, const QString &errorMsg);

public slots:
  void handleUploadResponse(const ChatSystem::Protocol::TransHeader &header,
                            const QByteArray &data);
  void handleDownloadResponse(const ChatSystem::Protocol::TransHeader &header,
                              const QByteArray &data);
