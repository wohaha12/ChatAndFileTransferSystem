...
NetworkManager::NetworkManager(QObject *parent)
    : QObject(parent), m_metaSocket(nullptr), m_storageSocket(nullptr),
      m_sequenceNumber(0) {}
...
bool NetworkManager::connectToMetaServer(const QString &ip, uint16_t port) {
  ...
  connect(m_metaSocket, &QTcpSocket::readyRead, this,
          &NetworkManager::onMetaSocketReadyRead);
  connect(m_metaSocket,
...
bool NetworkManager::connectToStorageServer(const QString &ip, uint16_t port) {
  ...
  connect(m_storageSocket, &QTcpSocket::readyRead, this,
          &NetworkManager::onStorageSocketReadyRead);
  connect(m_storageSocket,
          QOverload<QAbstractSocket::SocketError>::of(
              &QAbstractSocket::errorOccurred),
...
bool NetworkManager::sendToMetaServer(
    const ChatSystem::Protocol::TransHeader &header, const QByteArray &data) {
  if (!m_metaSocket ||
      m_metaSocket->state() != QAbstractSocket::ConnectedState) {
...
bool NetworkManager::sendToStorageServer(
    const ChatSystem::Protocol::TransHeader &header, const QByteArray &data) {
  if (!m_storageSocket ||
      m_storageSocket->state() != QAbstractSocket::ConnectedState) {
...
void NetworkManager::processReceivedData(QTcpSocket *socket, QByteArray &buffer,
                                         bool isMeta) {
  if (!socket) {
    return;
  }
  ...
  while (buffer.size() >=
    ...
    memcpy(&header, buffer.constData(),
           sizeof(ChatSystem::Protocol::TransHeader));
    ...
    uint32_t packetSize =
        sizeof(ChatSystem::Protocol::TransHeader) + header.len;
    ...
    QByteArray body =
        buffer.mid(sizeof(ChatSystem::Protocol::TransHeader), header.len);
